#!/usr/bin/env python3
# filepath: /Users/seiji/Documents/github/n8n-mcp-server/scripts/setup_mcp_linux.py
"""mcp.json への N8N / Ollama 認証情報登録スクリプト(Linux GUI: GNOME/KDE/XFce4)
...
def _assert_linux() -> None:
    if not sys.platform.startswith("linux"):
        raise SystemExit(
            f"このスクリプトは Linux 専用です。現在の OS: {sys.platform}"
        )
...
def _is_gnome() -> bool:
    de = _detect_de()
    return any(s in de for s in ("gnome", "cinnamon", "budgie"))


def _is_kde() -> bool:
    de = _detect_de()
    return any(s in de for s in ("kde", "plasma"))


def _is_xfce() -> bool:
    return "xfce" in _detect_de()
...
def resolve_target_path(explicit: str | None) -> Path:
    """優先順位:
        1. --out 明示指定
        2. HOME 配下の VS Code 系 mcp.json(既存時のみ採用)
           (Code / Code - Insiders / VSCodium / Cursor)
        3. ~/.vscode/mcp.json(ファイルの検索)
        4. どれも無ければ ~/.config/Code/User/mcp.json を新規作成
    """
    if explicit:
...
def _ask_zenity_form(title: str, defaults: Values) -> Values | None:
    """zenity --forms で 1 ウィンドウ 4 項目。"""
    ...
    try:
        proc = subprocess.run(
            [
                "zenity", "--forms",
                f"--title={title}",
                "--text=N8N / Ollama 認証情報を入力",
                "--width=520",
                "--columns=2",
                "--separator=\x1f",
                "--add-entry=N8N_URL",
                "--add-password=N8N_API_KEY",
                "--add-entry=OLLAMA_HOST",
                "--add-entry=OLLAMA_MODEL",
                "--hide-text",
            ],
...
def _ask_yad_form(title: str, defaults: Values) -> Values | None:
...
def _ask_kdialog_single(title: str, defaults: Values) -> Values | None:
    ...
    try:
        ...
        for label, hide in (
            ...
            if hide:
                cmd = ["kdialog", "--passwordbox", label]
            else:
                cmd = ["kdialog", "--inputbox", label, ""]
            proc = subprocess.run(
...
# =============================================================
# DE 別フォールバック順序(フル 1 ウィンドウ優先)
# =============================================================

def _candidates() -> list:
    order: list = []
    if _is_kde() and shutil.which("yad"):
        order.append(_ask_yad_form)
    elif _is_kde() and shutil.which("zenity"):
        order.append(_ask_zenity_form)
    elif _is_gnome():
        order.append(_ask_zenity_form)
    elif _is_xfce():
        order.append(_ask_yad_form)

    order.append(_ask_tk_form)

    for fn in (_ask_yad_form, _ask_zenity_form, _ask_kdialog_single):
        if fn not in order:
            order.append(fn)
    return order
...
def main() -> int:
    ...
    ap.add_argument(
        "--de", choices=["gnome", "kde", "xfce"],
        help="デスクトップ環境を強制指定(省略時は自動検出)",
    )
    args = ap.parse_args()
    ...
    print(f"デスクトップ環境: {_detect_de() or '(不明)'}")
    print(f"保存先: {target}")
    if not created:
        print("(既存ファイルに追記モード)")

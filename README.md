...
- **`[FATAL] 必須環境変数が未設定です` で起動に失敗する**
  ...
  検証ロジックとエラーメッセージのフォーマットは「起動時の環境変数検証」を確認の上で修正してください。

- **VS Code からの接続 / 手動で構成したい**
  スクリプト実行しても VS Code でツールが見つからない場合、
  `settings.json` または `mcp.json` のいずれかに以下を手動で追加します。

> VS Code 1.101+ では `mcp.json` の `servers` キーを使用します。
> それ未満のバージョンでは `settings.json` の `mcp.servers` キーを使用します。
...
| `destructiveHint` | 元に戻せない変更の可能性を事前告知する |

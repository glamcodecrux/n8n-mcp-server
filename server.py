...
def _validate_env() -> None:
    ...
    if missing:
        print(
            f"[FATAL] 必須環境変数が未設定です: {', '.join(missing)}\n"
...
async def ask_ollama(prompt: str, model: str = "") -> str:
    ...
    try:
        ...
        return json.dumps({"ok": False, "error": "http_status", "status_code": e.response.status_code, "message": e.response.text[:200]}, ensure_ascii=False)
...
if __name__ == "__main__":
    print(f"[INFO] n8n-mcp-server v{__version__}", file=sys.stderr)
    _validate_env()
    mcp.run()
